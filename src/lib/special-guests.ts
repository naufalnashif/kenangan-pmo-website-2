
export type SpecialGuest = {
    id: string;
    name: string;
    title: string;
    personalFarewell: string;
};

export const specialGuests: SpecialGuest[] = [
  {
    id: "Ka Widi",
    name: "Ka Widi",
    title: "Teman Research Fellow seperjuangan",
    personalFarewell: "Terima kasih sudah menjadi teman seperjuangan selama di Research Fellow. Banyak suka duka yang kita lewati bersama. Sukses selalu di manapun kamu berada!"
  },
  {
    id: "Aya",
    name: "Aya",
    title: "Teman Research Fellow seperjuangan",
    personalFarewell: "Terima kasih sudah menjadi teman seperjuangan selama di Research Fellow. Banyak suka duka yang kita lewati bersama. Sukses selalu di manapun kamu berada!"
  },
  {
    id: "Maria",
    name: "Maria",
    title: "Teman Research Fellow seperjuangan",
    personalFarewell: "Terima kasih sudah menjadi teman seperjuangan selama di Research Fellow. Banyak suka duka yang kita lewati bersama. Sukses selalu di manapun kamu berada!"
  },
  {
    id: "Mas Adit",
    name: "Mas Adit",
    title: "Atasan di PMOT OJK",
    personalFarewell: "Terima kasih banyak atas bimbingannya selama ini, terutama di proyek Data Profile, Launching FIND, dan Remake Dashboard Management Internal. Saya belajar banyak dari Mas Adit."
  },
  {
    id: "Mas Hendra",
    name: "Mas Hendra",
    title: "Atasan di PMOT OJK",
    personalFarewell: "Terima kasih banyak atas bimbingan dan kesempatannya, khususnya di proyek AI dan Remake Dashboard Management Internal. Pengalaman yang sangat berharga bagi saya."
  },
  {
    id: "Mba Susi",
    name: "Mba Susi",
    title: "Rekan, Senior di PMOT OJK",
    personalFarewell: "Terima kasih untuk setiap obrolan dan sharingnya, Mba. Selalu menyenangkan bisa berdiskusi banyak hal. Sehat dan sukses selalu, Mba Susi!"
  },
  {
    id: "Mba Dewi",
    name: "Mba Dewi",
    title: "Rekan, Senior di PMOT OJK",
    personalFarewell: "Walaupun Mba Dewi baru bergabung, senang bisa kenal dan ngobrol banyak akhir-akhir ini. Semoga sukses selalu di PMOT OJK, Mba!"
  },
  {
    id: "Mas Rudi",
    name: "Mas Rudi",
    title: "Rekan, Senior di PMOT OJK",
    personalFarewell: "Walaupun Mas Rudi baru bergabung, senang bisa kenal dan ngobrol banyak akhir-akhir ini. Semoga sukses selalu di PMOT OJK, Mas!"
  },
  {
    id: "Mas Aqsa",
    name: "Mas Aqsa",
    title: "Rekan, Senior di PMOT OJK",
    personalFarewell: "Terima kasih untuk bantuannya selama ini, terutama urusan payroll dan anggaran. Sukses di tempat barunya di OJK Institute, Mas!"
  },
  {
    id: "Pa Subhan",
    name: "Pa Subhan",
    title: "Rekan Senior di PMOT OJK",
    personalFarewell: "Terima kasih sudah menjadi teman makan, ngobrol, dan bercanda yang asyik. Momen-momen santai jadi lebih hidup. Sehat dan sukses selalu, Pa!"
  },
  {
    id: "Mba Sasti",
    name: "Mba Sasti",
    title: "Atasan, Senior di PMOT OJK",
    personalFarewell: "Senior saya yang murah senyum dan baik hati. Terima kasih untuk semua kolaborasi dan diskusinya. Semoga lancar terus di WS Hukum, Mba Sasti!"
  },
  {
    id: "Pa Jo",
    name: "Pa Jo",
    title: "Atasan, Senior di PMOT OJK",
    personalFarewell: "Beliau ini seperti ayah sendiri. Baik, murah senyum, teman ngobrol, diskusi, dan bercanda. Terima kasih untuk semua kebaikannya. Sehat selalu, Pa Jo!"
  },
  {
    id: "Pa Iren",
    name: "Pa Iren",
    title: "Kepala Unit PMO OJK",
    personalFarewell: "Terima kasih atas arahan, coaching, dan setiap canda tawanya, Pa. Saya ada tebak-tebakan nih: Kenapa zombie kalau nyerang bareng-bareng? ... Karena kalau sendiri namanya zomblo! Hehe. Sehat dan sukses selalu, Pa!"
  },
  {
    id: "Mas Eko",
    name: "Mas Eko",
    title: "Atasan, Senior di PMOT OJK",
    personalFarewell: "Terima kasih untuk semua diskusi dan kerjasamanya, Mas. Selamat dan sukses untuk peran barunya sebagai kepala kantor OJK di Belitung!"
  },
  {
    id: "Pa Inka",
    name: "Pa Inka",
    title: "Direktur Eksekutif WSSI",
    personalFarewell: "Terima kasih banyak atas kesempatan dan kepercayaan yang telah Bapak berikan. Bekerja di bawah arahan Bapak adalah suatu kehormatan. Sehat dan sukses selalu, Pa."
  },
  {
    id: "Pa Rifqi",
    name: "Pa Rifqi",
    title: "Deputi Direktur di PMOT OJK",
    personalFarewell: "Terima kasih atas semua kolaborasi dan diskusi kita selama ini. Saya sangat menghargai setiap momen dan pembelajaran yang ada. Sukses selalu, Pa Rifqi!"
  },
  {
    id: "Pa Agung",
    name: "Pa Agung",
    title: "Deputi Direktur di PMOT OJK",
    personalFarewell: "Terima kasih atas semua kolaborasi dan diskusi kita selama ini. Saya sangat menghargai setiap momen dan pembelajaran yang ada. Sukses selalu, Pa Agung!"
  },
  {
    id: "Pa Riza",
    name: "Pa Riza",
    title: "Atasan, Senior di PMOT OJK",
    personalFarewell: "Terima kasih atas semua kolaborasi dan diskusi kita selama ini. Saya sangat menghargai setiap momen dan pembelajaran yang ada. Sukses selalu, Pa Riza!"
  },
  {
    id: "Moses",
    name: "Moses",
    title: "Teman Research Fellow seperjuangan",
    personalFarewell: "Walaupun sudah pindah duluan ke BRI, jejak perjuangan kita di RF tetap terkenang. Sukses terus di sana, Mos!"
  },
  {
    id: "Naufal Nashif",
    name: "Naufal Nashif",
    title: "The one and only",
    personalFarewell: "Ini adalah sebuah refleksi dari perjalanan yang telah dilalui. Semoga apa yang ditinggalkan bisa bermanfaat dan kenangan baik akan selalu ada."
  },
  {
    id: "Mba Ode",
    name: "Mba Ode",
    title: "Kepala Bagian di PMOT",
    personalFarewell: "Terima kasih banyak atas kebaikan dan setiap obrolan kita, Mba. Senang bisa mengenal Mba Ode. Sehat dan sukses selalu untuk Mba dan keluarga."
  },
  {
    id: "Gigih",
    name: "Gigih",
    title: "Rekan di DIPM (PCS 8)",
    personalFarewell: "Thank you brother, sampai jumpa lagi, semoga sehat dan sukses selalu bray!"
  },
  {
    id: "Mas Ghozy",
    name: "Mas Ghozy",
    title: "Rekan Senior di DPEA",
    personalFarewell: "Makasih mas untuk pertemuan singkat yang berkesan, sampai jumpa lagi, semoga sehat dilancarkan setiap kegiatannya, sukses selalu mas!"
  },
];
